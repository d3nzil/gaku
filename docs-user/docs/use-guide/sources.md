...
# Creating a source
To create a new source, go to `Card Manager` - `Sources` section. In the `Add source` section there will be two text inputs - `Source Name` and `Source Section`. The name is intended for the name of the book or other source - e.g "Ranma 1/2", the section for specific book and chapter - e.g. "Book 1, Chapter 2". Only the name is required.
